...
## ------------------------------------------------------------
## 3) Panel a: DIC concentration (was CO2, now split from CH4)
## ------------------------------------------------------------
...
## ------------------------------------------------------------
## 7) Panel e: alpha (finer y-axis breaks)
## ------------------------------------------------------------
...
whiticar_panel <- ggplot() +
  ...
  scale_shape_manual(
    values = site_shapes,
    name = "Site",
    labels = site_labels,     # long names in legend only
    drop = FALSE
  ) +
  scale_x_continuous(
    limits = c(-101, -20),
...
## ------------------------------------------------------------
## 9) Final multi-panel figure a-f
##    (a) DIC (b) CH4 (c) δ13C-CO2 (d) δ13C-CH4 (e) alpha (f) whiticar
## ------------------------------------------------------------
...
overall <- (
  ...
  plot_annotation(tag_levels = "a") &     # lowercase panel tags a-f
  theme(
    legend.position = "right",
...
# Generic helper: full pairwise Wilcoxon + direction, for one variable
...
# Run for all five variables shown in Figure 4
