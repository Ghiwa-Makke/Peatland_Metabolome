...
site_levels <- c("S1", "BLF", "S3", "US2", "LC")   # T1 -> S1

# short codes for internal joins/facets; full names shown in-plot and in legends
site_labels <- c(
  "S1"  = "S1-bog",
  "BLF" = "BLF-poor fen",
  "S3"  = "S3-fen",
...
site_colors <- c(
  "S1"  = "#8B4513",   # bog - brown
  "BLF" = "#F5DEB3",   # poor fen - amber
  "S3"  = "#F5DEB3",   # minerotrophic fen - green
  "US2" = "#F5DEB3",   # fen - purple
  "LC"  = "#2F8B8B"    # shrub wetland - teal
)
...
make_mode_plot <- function(prepped, mode_title, fill_limits, show_legend = FALSE) {
  ...
  p_hm <- ggplot(prepped$hm_df, aes(x = SampleID, y = Group, fill = Intensity_plot)) +
    ...
    scale_fill_viridis_c(
      ...
      limits    = fill_limits,
      oob       = scales::squish,
      name      = expression(log[10] ~ "(mean intensity)")   # subscript 10
    ) +
    scale_x_discrete(drop = FALSE) +
...
# =========================================================
# 3b) CLASS-LEVEL STATISTICS (Sup. Table 3)
#     Mirrors run_site_pairwise_stats() from Figure 4:
#     medians, direction, and BH-corrected pairwise comparisons.
# =========================================================
...
# ---- Table A: ALL pairwise comparisons, medians + direction (matches Fig. 4 format) ----
...
write_csv(sup_table3_all,     "Plots/New/SupTable3_class_pairwise_ALL.csv")
write_csv(sup_table3_sig,     "Plots/New/SupTable3_class_pairwise_SIGNIFICANT.csv")
write_csv(sup_table3_omnibus, "Plots/New/SupTable3_class_kruskalwallis_omnibus.csv")

# =========================================================
# 4) SCFA PANEL WITH SIGNIFICANCE
# =========================================================
...
## ── colored site strips: extended site names on facet strip ─────────────────
...
ggsave("Plots/New/Figure5_final_with_SCFA.svg", final_figure5,
