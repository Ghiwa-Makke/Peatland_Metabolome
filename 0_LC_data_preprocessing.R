...
#HILIC -----
### input data:
cd_results_table <- readxl::read_xlsx("Data/LC/SPRUCE_2023_Surface_HILIC_CD_Compounds_Table.xlsx")
...
metadata <- read_csv("Data/LC/SPRUCE_2023_Surface_metadata_HILIC.csv")
...
# RP -----
### input data:
cd_results_table <- readxl::read_xlsx("Data/LC/SPRUCE_2023_Surface_RP_CD_Compounds_Table.xlsx")
# Select columns needed for downstream analysis
cd_results_table <- cd_results_table %>%
  arrange(desc(`Calc. MW`)) %>% 
  select(FeatureID, Name, Formula, `Calc. MW`, contains('Annotation source'), contains('Results'), contains('Area:'), contains('Gap Status:'), contains('Gap Fill Status:')) %>% 
...
# Import metadata and fix names
metadata <- read_csv("Data/LC/SPRUCE_2023_Surface_metadata_RP.csv")
